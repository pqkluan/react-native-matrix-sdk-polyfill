...
function keyEndToEndSession(deviceKey: string, sessionKey: string): string {
  ...
  return `${E2E_PREFIX}sessions/${encodedDeviceKey}/${encodedSessionKey}`;
}

function keyPrefixEndToEndSession(deviceKey: string): string {
  const encodedDeviceKey = encodeURIComponent(deviceKey);
  return `${E2E_PREFIX}sessions/${encodedDeviceKey}/`;
}

function keyEndToEndSessionProblems(deviceKey: string, sessionKey: string): string {
  const encodedDeviceKey = encodeURIComponent(deviceKey);
  const encodedSessionKey = encodeURIComponent(sessionKey);
  return `${E2E_PREFIX}session.problems/${encodedDeviceKey}/${encodedSessionKey}`;
}

function keyEndToEndInboundGroupSession(senderKey: string, sessionId: string): string {
  const encodedSenderKey = encodeURIComponent(senderKey);
  const encodedSessionId = encodeURIComponent(sessionId);
  return `${KEY_INBOUND_SESSION_PREFIX}${encodedSenderKey}/${encodedSessionId}`;
}

function keyEndToEndInboundGroupSessionWithheld(senderKey: string, sessionId: string): string {
  const encodedSenderKey = encodeURIComponent(senderKey);
  const encodedSessionId = encodeURIComponent(sessionId);
  return KEY_INBOUND_SESSION_WITHHELD_PREFIX + encodedSenderKey + '/' + encodedSessionId;
}
...
function keyOutgoingKeyRequestPrefix(requestId: string): string {
...
export default class AsyncCryptoStore {
  ...
  async getEndToEndSessionProblem(deviceKey, timestamp) {
    ...
    for (const problem of problems)
      if (problem.time > timestamp) return Object.assign({}, problem, { fixed: lastProblem.fixed });

    if (lastProblem.fixed) return null;
    else return lastProblem;
  }
  ...
  async deleteAllData() {
    ...
    for (const k of allE2eStorageKeys) await this.asyncStorage.removeItem(k);
  }
  ...
  async getOutgoingRoomKeyRequestByState(wantedStates) {
    ...
    for (const reqId of allRequestIds) {
      const req = await this._getJsonItem(reqId);
      if (wantedStates.includes(req.state)) return req;
    }
    return null;
  }
  ...
  async getAllOutgoingRoomKeyRequestsByState(wantedStates) {
    ...
    for (const reqId of allRequestIds) {
      const req = await this._getJsonItem(reqId);
      if (wantedStates.includes(req.state)) reqs.push(req);
    }
    return reqs;
  }
  ...
  async updateOutgoingRoomKeyRequest(requestId, expectedState, updates) {
    ...
    if (!req || req.state !== expectedState) return;
  ...
  async deleteOutgoingRoomKeyRequest(requestId, expectedState) {
    ...
    const req = await this._getJsonItem(storeKey);
    if (req.state !== expectedState) return;
